...
function showDigestWaiting(){const time=localStorage.getItem('pinmind.dailyTime')||'22:00',reached=dailyTimeReached(),date=document.querySelector('.date');if(date)date.textContent=reached?'正在生成':'等待生成';if(reached)showDigestState('正在整理今日知识','只会整理上次生成后新保存的链接、文字和截图。');else showDigestState(`今日知识将在 ${time} 生成`,'');}
