...
(function enablePinMindDemo(){
  ...
  PinMindAPI.today=async()=>{try{const live=await original.today();if(live.knowledge_items?.length)return live}catch{}const active=window.dailyTimeReached?.()||false,current=window.PinMindSchedule?.localDateKey(new Date())||today;return {digest_date:current,knowledge_items:active?[PINMIND_DEMO_KNOWLEDGE[2]]:[]}};
