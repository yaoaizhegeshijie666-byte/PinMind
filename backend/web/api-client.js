...
window.mapKnowledgeItems=mapKnowledgeItems;async function loadLiveDigest(){
  ...
  try{
    ...
    const now=new Date(),today=now.getFullYear()+'-'+String(now.getMonth()+1).padStart(2,'0')+'-'+String(now.getDate()).padStart(2,'0');if(data.digest_date===today&&!window.PinMindDemo?.enabled&&!window.dailyTimeReached?.()){window.showDigestWaiting?.();return;}if(!data.knowledge_items?.length){window.showDigestState?.('今天没有可生成的知识','上次生成后没有新增的有效链接、文字或截图。');return;}
...
window.addEventListener('DOMContentLoaded',async()=>{await syncNativeCaptures();if(window.PinMindDemo?.enabled)await loadLiveDigest();else if(window.dailyTimeReached?.())await refreshDailyDigest();else await loadLiveDigest();setTimeout(()=>checkClipboardLink(),900);document.querySelector('#pasteLinkButton')?.addEventListener('click',()=>checkClipboardLink(true));});
