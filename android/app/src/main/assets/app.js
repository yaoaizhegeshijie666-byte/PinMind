...
function showDigestState(title,message){window.currentKnowledgeItems=[];window.viewingToday=true;list.hidden=false;list.innerHTML=`<section class="empty-state"><span>◷</span><h2>${escapeHtml(title)}</h2>${message?`<p>${escapeHtml(message)}</p>`:''}</section>`;const introSection=document.querySelector('.intro');if(introSection)introSection.hidden=false;const intro=introSection?.querySelector('p');if(intro)intro.textContent=message||'';const button=document.querySelector('#readButton');if(button)button.hidden=true;}
